...
- Custom persistent-secret wallet Rev A requirements validation, requiring
  ...
  power cycling.
- Custom persistent-secret wallet key-material lifecycle validation against the
  shared specs `persistent-secret-custody-v0` fixture, requiring no plaintext
  secret at rest, wrapped or encrypted storage, TROPIC01-assisted unlock into
  ESP32-S3 RAM, RAM-only plaintext, mandatory wipe events, MAC-and-Destroy PIN
  attempt hardening, and disabled-by-default backup/export behind device
