...
| C3 | Backlight boost drives **4 parallel** LEDs from a 4.4 V node → overdrive, can't switch off | replace TPS61165 with a **low-Vin WLED driver, PWM-dimmed**. ⚠️ **NOT AL8860** (verified: min VIN 4.5 V > SYS_PWR_IN's 4.4 V → never turns on) and **not a plain boost** (can't regulate down at 4.4 V). Requirement: drive 4 parallel Vf-3.2 V/80 mA strings from SYS_PWR_IN **2.9–4.4 V** with PWM. Correct class = a **charge-pump/buck-boost WLED driver with current sinks** (constant brightness across the Li range) — or a linear 4-sink accepting graceful dim below ~3.5 V. **Reselect the exact MPN (needs its own check — this is the one review fix that was itself mis-specified).** |
