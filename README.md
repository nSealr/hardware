...
- Machine-readable custom persistent-secret wallet Rev A requirements and BOM
  ...
  `persistent-secret-custody-v0` key-material lifecycle forbids plaintext
  secrets at rest, requires wrapped or encrypted storage before persistent
  custody is enabled, limits plaintext to ESP32-S3 RAM after TROPIC01-assisted
  unlock, and requires wipe events plus device-reviewed physical approval for
  backup/export paths.
...
- Validation script and tests for required interfaces, security/review
  ...
  boundaries, specs-backed key-material lifecycle gates, rejection of dedicated
  TROPIC01 reset-pin modeling, QR vault envelope/review/approval requirements,
